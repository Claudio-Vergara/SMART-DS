...
M=lower(M);
xx=length(M);
summary_lines_final.swres=zeros(length(summary_lines_final.name),1);
for kk=1:xx
MM = strsplit(M{kk,1});
if length(MM)>7 && strcmp(MM{1,7},'r1=1e11')
    ...
    ix1=strcmp(summary_lines_final.from,frm_bus);
    ix2=find(ix1==1);
    ix3=strcmp(summary_lines_final.to,to_bus);
    ix4=find(ix3==1);
    aares=intersect(ix2,ix4);
    summary_lines_final.swres(aares)=1e11;
end
end

% ixres = find(summary_lines_final.swres > 1000);
% sw_frm = summary_lines_final.from(ixres);
% sw_to = summary_lines_final.to(ixres);
% xz=length(ixres);
% 
% for xa=1:xz  
% ix1=strcmp(summary_lines_final.from,sw_to(xa));
% ix2=find(ix1==1);
% btwn_to = summary_lines_final.to(ix2);
% xy=length(btwn_to);
% for xb=1:xy
%     ix3=strcmp(summary_lines_final.from,btwn_to(xb));
%     ix4=find(ix3==1);
%     for xc = 1:length(ix4)
%         if strcmp(summary_lines_final.type(ix4(xc)),'switch') && (summary_lines_final.swres(ix4(xc)) > 1000)
%             lin_dis_to = btwn_to(xb)
%             lin_dis_frm = sw_to(xa)
%             Idx1 = strcmp(bus_names,strtok(lin_dis_frm,'.'));
%             Idx2 = find(Idx1==1);
%             Idx3 = strcmp(bus_names,strtok(lin_dis_to,'.'));
%             Idx4 = find(Idx3==1);
%             a_matrix1(Idx2,Idx4) = 0;
%             a_matrix1(Idx4,Idx2) = 0;
%         end
...
% for xxa=1:No_of_SubGraphs
%     iii=length(find(C==xxa));
%     if iii<3
%         idxx=find(C==xxa);
%         a_matrix1(idxx(1),idxx(2))=0;
%         a_matrix1(idxx(2),idxx(1))=0;
%     end
% end
...
for iz=1:No_of_SubGraphs
    ...
    [Idz_from,ia_frm,ib_frm] = intersect(lin_from,bus_names(Id_bus{iz}));
    
    [Idz_to,ia_to,ib_to] = intersect(lin_to,bus_names(Id_bus{iz}));
    
    [Idz_nod,ia_nod,ib_nod] = intersect(nod_bus,bus_names(Id_bus{iz}));
    
    [Idz_line,ia,ib] = unique([ia_frm;ia_to]);
    
    Idz_line = Idz_line(ib);
    
    res = max(summary_lines_final.swres(Idz_line,:));
    
    if iz == C(1) || (res > 1000 && length(ia_nod)<3)
        1;
    else
        iz2=iz2+1;
        filename = sprintf('%s_%d','FeederNo',iz2);
        summary_feeder_lines.(filename) = summary_lines_final(Idz_line,:);
        summary_feeder_nodes.(filename) = summary_nodes_final(ia_nod,:);
...
Feeder_Summary1=Feeder_Final_Summary;
