function [network] = agg_metrics_ODSS(file, summary_nodes_table, meter_summary,...
    summary_lines_table, x_load, inputsTable, circuit, a_matrix_non)
...
% First, calculate metrics/stats specific to the MV network
    ...
    V_levels_vsource = inputsTable{vsource_rows, 'basekV'};
...
%% Caculate LV specific Metrics: cycle through each MV/LV tansformer, assuming that it is the starting point of a LV network
    ...
    for iLV = 1:length(MVLV_x) 
        ...
        if file.data_type == 1 % GLD
            config_row = find(strcmp(x_table.rowLabels, 'configuration'));
            name_row = find(strcmp(x_table.rowLabels, 'name'));
            xfm_names = x_table{name_row, :};
            xformer_col = find(strcmp(xfm_names, name));
            this_config = x_table{config_row, xformer_col};
            ...
            xfm_2ndV(iLV) = str2double(second_V);
...
%%    Roll-Up LV metrics to MV level and save to network structure
    ...
    network.MV(iMV).V_levels.LV = unique(array_2ndV); % total LV levels
...
%     final_name_nodes = {bus_names{connected_col,1}}';
    ...
    network.topology.clust_coeff = clust_coeff; % clustering coeff
