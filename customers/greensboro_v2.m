...
switch pType
    ...
    [M,I]= min(abs(totArea-buildingTypesData{resIndices,4}));
    users.p(i)=buildingTypesData{resIndices(I),9};
    case 2 % commercial
        comIndices=[1,7,9];
        [M,I]= min(abs(totArea-buildingTypesData{comIndices,4}));
        users.p(i)=buildingTypesData{comIndices(I),9};
    case 3 % office
        ofIndices=[4,5,12];
        [M,I]= min(abs(totArea-buildingTypesData{ofIndices,4}));
        users.p(i)=buildingTypesData{ofIndices(I),9};
    case 4 % industrial
        users.p(i)=0.1*totArea; %guess for industrial specific load
    case 5 % hotel
        hotIndices=[3,11];
    [M,I]= min(abs(totArea-buildingTypesData{hotIndices,4}));
    users.p(i)=buildingTypesData{hotIndices(I),9};
    ...
    case 7 % retail
        retIndices=[13,14,15];
        [M,I]= min(abs(totArea-buildingTypesData{retIndices,4}));
        users.p(i)=buildingTypesData{retIndices(I),9};
    case 8 % schools
    schIndices=[8,10];
    [M,I]= min(abs(totArea-buildingTypesData{schIndices,4}));
    users.p(i)=buildingTypesData{schIndices(I),9};
    
    case 9 % hospitals
    hospIndices=[8,10];
    users.p(i)=buildingTypesData{2,9};
